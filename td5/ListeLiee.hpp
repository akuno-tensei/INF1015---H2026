/*
		...
		Description : Ce fichier contient la définition de la classe ListeLiee, 
					  qui est une implémentation d'une liste doublement liée.  
					  La classe gère les opérations de base telles que l'ajout, 
					  l'insertion et la suppression d'éléments, ainsi que la gestion de la mémoire.
*/
...
public:
	...
	void push_back(const T& item)
	{
		...
		auto nouveauNoeud = std::make_unique<Noeud<T>>(item);
		Noeud<T>* ptrNouveauNeoud = nouveauNoeud.get();

		if (estVide()) {
			tete_ = std::move(nouveauNoeud);
			queue_ = ptrNouveauNeoud;
		}
		else {
			nouveauNoeud->precedent_ = queue_;
			queue_->suivant_ = std::move(nouveauNoeud);
			queue_ = ptrNouveauNeoud;
		}
		taille_++;
	}
	...
	iterator insert(iterator it, const T& item)
	{
		...
		auto nouveauNoeud = std::make_unique<Noeud<T>>(item);
		Noeud<T>* cible = it.position_;
		Noeud<T>* precedent = cible->precedent_;

		nouveauNoeud->suivant_ = std::move(precedent->suivant_);
		nouveauNoeud->precedent_ = precedent;

		cible->precedent_ = nouveauNoeud.get();
		precedent->suivant_ = std::move(nouveauNoeud);

		taille_++;

		return iterator(cible->precedent_);
	}
	...
	iterator erase(iterator it)
	{
		...
		Noeud<T>* cible = it.position_;
		Noeud<T>* precedent = cible->precedent_;
		Noeud<T>* suivant = cible->suivant_.get();

		if (precedent == nullptr && suivant == nullptr) {
			tete_ = nullptr;
			queue_ = nullptr;
		}
		else if (precedent == nullptr && suivant != nullptr) {
			suivant->precedent_ = nullptr;
			tete_ = std::move(cible->suivant_);
		}
		else if (precedent != nullptr && suivant == nullptr) {
			precedent->suivant_ = nullptr;
			queue_ = precedent;
		}
		else {
			precedent->suivant_ = std::move(cible->suivant_);
...
private:
	unique_ptr<Noeud<T>> tete_;  //NOTE: Vous pouvez changer le type si vous voulez.
	Noeud<T>* queue_;
	unsigned taille_;
};
